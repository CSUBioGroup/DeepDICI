...
class DeepDICI(nn.Module):
    def __init__(self, hp, protein_MAX_LENGH=1000, drug_MAX_LENGH=100):
        super(DeepDICI, self).__init__()
        self.drug_dim = hp.drug_dim
        self.protein_dim = hp.protein_dim
        self.char_dim = hp.char_dim
        self.conv = hp.conv
        ...
        self.hidden_dim = 512
        self.output_dim = 64
        self.drug_reduction = nn.Linear(self.drug_dim, self.char_dim)
        ...
        self.protein_reduction = nn.Linear(self.protein_dim, self.char_dim)
    ...
    def forward(self, drug, protein):
        drugembed = self.drug_reduction(drug)
        drugembed = drugembed.permute(0, 2, 1)
        proteinembed = self.protein_reduction(protein)
