...
def show_result(DATASET, lable, Accuracy_List, Precision_List, Recall_List, AUC_List, AUPR_List):
    ...
    with open(
        "./result/" + INFO + "/" + DATASET + "/" + current_time + "/results.txt", "w"
    ) as f:
...
def test_model(dataset_load, save_path, DATASET, LOSS, dataset="Train", lable="best", save=True):
    test_pbar = tqdm(
        enumerate(BackgroundGenerator(dataset_load)), total=len(dataset_load)
    )
    ...
    if save:
        with open(
            save_path + "/{}_{}_{}_prediction.txt".format(DATASET, dataset, lable), "a"
        ) as f:
            for i in range(len(T)):
                f.write(str(T[i]) + " " + str(P[i]) + "\n")
    results = "{}_set--Loss:{:.5f};Accuracy:{:.5f};Precision:{:.5f};Recall:{:.5f};AUC:{:.5f};PRC:{:.5f}.".format(
        lable, loss_test, Accuracy_test, Precision_test, Recall_test, AUC_test, PRC_test
    )
    print(results)
...
if __name__ == "__main__":
    ...
    DATASET = "ionchannel_dataset"
    required_files = [
        f"{DATASET}.pkl",
        "embedding/proteins_dict.pkl",
        "embedding/drug_dict.pkl"
    ]
    for file in required_files:
        file_path = os.path.join(data_dir, file)
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"Required file not found: {file_path}")
    current_time = hp.current_time
    print("INFO:"+INFO)
    print("Train in " + DATASET)
    weight_CE = None
    print("load data")
    with open(os.path.join(data_dir, f"{DATASET}.pkl"), "rb") as f:
        splits = pickle.load(f)
    print("load finished")
    protein_feature_dict = {}
    drug_feature_dict = {}
    with open(os.path.join(data_dir, "embedding/proteins_dict.pkl"), "rb") as f:
        protein_feature_dict = pickle.load(f)
    with open(os.path.join(data_dir, "embedding/drug_dict.pkl"), "rb") as f:
        drug_feature_dict = pickle.load(f)
    print("Protein feature dict loaded, protein count:", len(protein_feature_dict))
    print("Drug feature dict loaded, drug count:", len(drug_feature_dict))
    print("data shuffle")
    K_Fold = 5
    (
    ...
    for i_fold in range(K_Fold):
        ...
        fold_data = splits[f"fold_{i_fold}"]
        train_dataset = CustomDataSet(
            fold_data["train"], protein_feature_dict, drug_feature_dict
        )
        valid_dataset = CustomDataSet(
            fold_data["valid"], protein_feature_dict, drug_feature_dict
        )
        test_dataset = CustomDataSet(
            fold_data["test"], protein_feature_dict, drug_feature_dict
        )
        ...
        early_stopping = EarlyStopping(
            savepath=save_path, patience=hp.Patience, verbose=True, delta=1e-4
        )
        print("Training...")
        ...
        for epoch in range(1, hp.Epoch + 1):
            ...
            print_msg = (
                f"[{epoch:>{epoch_len}}/{hp.Epoch:>{epoch_len}}] "
        ...
        (
            ...
            test_dataset_load, save_path, DATASET, Loss, dataset="Test", lable="stable"
        )
